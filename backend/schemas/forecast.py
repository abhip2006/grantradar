...
class ForecastGrant(BaseModel):
    ...
    historical_deadline_month: Optional[int] = Field(None, ge=1, le=12, description="Historical deadline month")
    recurrence_pattern: str = Field(default="annual", description="Pattern: annual, biannual, quarterly")
    ...
    match_score: Optional[float] = Field(None, ge=0, le=1, description="Match score with user profile")
    reasoning: Optional[str] = Field(None, description="Why this grant is recommended")
    fiscal_quarter: Optional[int] = Field(None, ge=1, le=4, description="Federal fiscal quarter (1-4)")
    is_federal_funder: bool = Field(default=False, description="Whether this is a federal funding agency")
...
class SeasonalTrend(BaseModel):
    ...
    top_categories: list[str] = Field(default_factory=list, max_length=5, description="Top grant categories")
    top_funders: list[str] = Field(default_factory=list, max_length=5, description="Top funding agencies")
...
class SeasonalTrendResponse(BaseModel):
    ...
    peak_months: list[int] = Field(default_factory=list, description="Months with highest activity")
...
class RecommendationGrant(BaseModel):
    ...
    match_reasons: list[str] = Field(default_factory=list, description="Reasons for the recommendation")
    profile_overlap: list[str] = Field(default_factory=list, description="Overlapping profile areas")


class RecommendationsResponse(BaseModel):
    """Response schema for AI recommendations."""

    recommendations: list[RecommendationGrant] = Field(..., description="Personalized recommendations")
...
class DeadlineHistoryStatsResponse(BaseModel):
    ...
    top_funders: list[dict] = Field(default_factory=list, description="Top funders by record count")
...
class FunderPrediction(BaseModel):
    ...
    typical_day_of_month: Optional[int] = Field(None, ge=1, le=31, description="Typical day of month for deadlines")
    typical_months: list[int] = Field(default_factory=list, description="Months when deadlines typically occur")
    ...
    grant_titles: list[str] = Field(default_factory=list, description="Sample grant titles")
