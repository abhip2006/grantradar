...
def upgrade() -> None:
    """Create review workflow tables (idempotent)."""
    all_exist = (
        table_exists("review_workflows") and
        table_exists("application_reviews") and
        table_exists("review_stage_actions") and
        table_exists("application_team_members")
    )
    if all_exist:
        return

    # Create review_workflows table
    if not table_exists("review_workflows"):
        op.create_table(
            "review_workflows",
    ...
    if not table_exists("application_reviews"):
        op.create_table(
            "application_reviews",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
    ...
    if not table_exists("review_stage_actions"):
        op.create_table(
    ...
    if not table_exists("application_team_members"):
        op.create_table(
