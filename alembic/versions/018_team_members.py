...
def upgrade() -> None:
    """Create team member tables (idempotent)."""
    if table_exists("lab_members") and table_exists("application_assignees"):
        return

    # Create lab members table
    if not table_exists("lab_members"):
        op.create_table(
            "lab_members",
    ...
    if not table_exists("application_assignees"):
        op.create_table(
