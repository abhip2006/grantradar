...
def upgrade() -> None:
    """Create custom fields tables (idempotent)."""
    if table_exists("custom_field_definitions") and table_exists("custom_field_values"):
        return

    # Create custom field definitions table
    if not table_exists("custom_field_definitions"):
        op.create_table(
    ...
    if not table_exists("custom_field_values"):
        op.create_table(
            "custom_field_values",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
