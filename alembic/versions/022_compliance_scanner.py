...
def upgrade() -> None:
    """Create compliance scanner tables (idempotent)."""
    if table_exists("compliance_rules") and table_exists("compliance_scans"):
        return

    # Create compliance_rules table
    if not table_exists("compliance_rules"):
        op.create_table(
            "compliance_rules",
    ...
    if not table_exists("compliance_scans"):
        op.create_table(
            "compliance_scans",
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
