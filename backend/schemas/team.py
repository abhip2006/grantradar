...
class TeamInviteRequest(BaseModel):
    ...
    role: MemberRole = Field(default=MemberRole.MEMBER, description="Role to assign to the new member")
    message: Optional[str] = Field(None, max_length=1000, description="Optional personal message for the invitation")
...
class TeamMemberBase(BaseModel):
    ...
    permissions: Optional[MemberPermissions] = Field(None, description="Member's permissions")
...
class TeamMemberResponse(BaseModel):
    ...
    member_email: str = Field(..., description="Email address of the team member")
    member_user_id: Optional[UUID] = Field(None, description="User ID if member has registered")
    ...
    invitation_expires_at: Optional[datetime] = Field(None, description="When the invitation expires")
    declined_at: Optional[datetime] = Field(None, description="When the invitation was declined")
    permissions: Optional[MemberPermissions] = Field(None, description="Member's role-based permissions")
    member_name: Optional[str] = Field(None, description="Name of the member if registered")
...
class TeamActivityResponse(BaseModel):
    ...
    actor_id: Optional[UUID] = Field(None, description="ID of the user who performed the action")
    ...
    entity_name: Optional[str] = Field(None, description="Name or description of the affected entity")
    metadata: Optional[dict[str, Any]] = Field(None, description="Additional metadata about the activity")
    ...
    actor_email: Optional[str] = Field(None, description="Email of the user who performed the action")
...
class TeamActivityFilters(BaseModel):
    ...
    action_types: Optional[List[ActivityType]] = Field(None, description="Filter by action types")
    ...
    from_date: Optional[datetime] = Field(None, description="Activities from this date onwards")
    to_date: Optional[datetime] = Field(None, description="Activities until this date")
...
class TeamStatsResponse(BaseModel):
    ...
    active_members: int = Field(..., description="Number of members who accepted invitations")
    pending_invitations: int = Field(..., description="Number of pending invitations")
    declined_invitations: int = Field(..., description="Number of declined invitations")
    expired_invitations: int = Field(..., description="Number of expired invitations")
    members_by_role: dict[str, int] = Field(..., description="Count of members per role")
...
class PermissionTemplatePermissions(BaseModel):
    ...
    can_delete: bool = Field(default=False, description="Can delete applications")
    can_invite: bool = Field(default=False, description="Can invite other team members")
    can_manage_grants: bool = Field(default=False, description="Can manage grant submissions and tracking")
...
class PermissionTemplateCreate(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="Template name for identification")
    description: Optional[str] = Field(None, max_length=500, description="Description of when to use this template")
    permissions: PermissionTemplatePermissions = Field(..., description="Permission configuration for this template")
    is_default: bool = Field(default=False, description="Set as default template for new invitations")


class PermissionTemplateUpdate(BaseModel):
    """Request to update a permission template."""

    name: Optional[str] = Field(None, min_length=1, max_length=100, description="Updated template name")
    description: Optional[str] = Field(None, max_length=500, description="Updated description")
    permissions: Optional[PermissionTemplatePermissions] = Field(None, description="Updated permissions")
...
class PermissionTemplateResponse(BaseModel):
    ...
    permissions: PermissionTemplatePermissions = Field(..., description="Permission configuration")
    ...
    member_count: Optional[int] = Field(None, description="Number of members using this template")
...
class BulkInviteItem(BaseModel):
    ...
    permission_template_id: Optional[UUID] = Field(None, description="Permission template to apply")


class BulkInviteRequest(BaseModel):
    """Request to send multiple team invitations at once."""

    invites: List[BulkInviteItem] = Field(..., min_length=1, max_length=50, description="List of invitations to send")
    message: Optional[str] = Field(None, max_length=1000, description="Personal message to include in all invitations")
...
class BulkInviteResultItem(BaseModel):
    ...
    member: Optional[TeamMemberResponse] = Field(None, description="Created member record if successful")
...
class BulkInviteResponse(BaseModel):
    ...
    results: List[BulkInviteResultItem] = Field(..., description="Individual results for each invitation")
