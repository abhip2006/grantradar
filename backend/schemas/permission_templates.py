...
class TemplatePermissions(BaseModel):
    ...
    can_manage_grants: bool = Field(
        default=False, description="Can manage grant pipeline"
    )
...
class PermissionTemplateCreate(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="Template name for display"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Description of when to use this template"
    )
    permissions: TemplatePermissions = Field(
        ..., description="Permission settings for this template"
    )
...
class PermissionTemplateUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Template name for display"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Description of when to use this template"
    )
    permissions: Optional[TemplatePermissions] = Field(
        None, description="Permission settings for this template"
    )
    is_default: Optional[bool] = Field(
        None, description="Whether this is the default template for new invitations"
    )
...
class PermissionTemplateListResponse(BaseModel):
    ...
    templates: List[PermissionTemplateResponse] = Field(
...
class ApplyTemplateResponse(BaseModel):
    ...
    permissions: TemplatePermissions = Field(
        ..., description="Permissions applied to the member"
    )
